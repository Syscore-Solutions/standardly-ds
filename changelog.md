...
- **Assessed Cloudflare Pages readiness** (Naman's question, ahead of S1-T11): the repo-root dashboard deploys zero-config (static, all four fetched files tracked), with three caveats recorded in `memory.md §7` — `main` lacks `index.html` until this branch merges, deploying the repo root makes every tracked doc public (memory/brief/architecture included), and the explorations app needs its own build config. Cloudflare Pages added to the S1-T11 candidate list alongside Vercel/Netlify.
- Docs re-synced to HEAD after the dashboard rewire: `memory.md` updated (repo map now lists `index.html`, S1 state covers the o1 wiring + preview-surface removal, CF Pages notes); refreshed the dashboard's embedded tasks/roadmap/changelog snapshot (drifted since the pre-chunk-8 build; the tokens block was verified in sync — it intentionally strips `$extensions` and minifies).
